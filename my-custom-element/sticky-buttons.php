...
<script>
    ...
    window.onload = function() {
        ...
        const btnBackground = "<?php echo $sticky_buttons_cropped_background ?>";
        const btnFullSizeBackground = "<?php echo $sticky_buttons_full_size_background ?>";
        ...
        const croppedBtnUrls = <?php echo $buttons_urls_json; ?>;
        const fullSizeUrls = <?php echo $full_size_buttons_urls_json; ?>;
        ...
        const customStickyTiles = document.querySelector('.custom-sticky-tiles');
        const customImageButtons = document.querySelectorAll(".custom-image-button");
        ...
        setTimeout(() => {
            ...
            if (desktop && containerPageHeader && containerPageHeader.offsetHeight !== undefined) {
                offsetTop = containerTiles.offsetTop + containerPageHeader.offsetHeight;
            } else if (desktop && !containerPageHeader) {
                offsetTop = containerTiles.offsetTop;
            } 

            if (mobile && containerPageHeader && containerPageHeader.offsetHeight !== undefined) {
                offsetTop = containerTiles.offsetTop - containerPageHeader.offsetHeight - containerMasthead.offsetHeight;
            } else if (mobile && !containerPageHeader && isStuckMasthead && adminBar) {
            ...
            } else if (mobile && !containerPageHeader && !isStuckMasthead) {
                offsetTop = containerTiles.offsetTop;
            }
        }, 500);
        ...
        if (stickyButtonsFullSize === 'true' && stickyButtonsDropdown !== "true") {
            tilesCropped.style.display = 'none';
            tilesFullSize.style.display = 'flex';
            stickyHeadContainer.style.display = "flex"
        } else if (stickyButtonsFullSize === 'true' && stickyButtonsDropdown === "true") {
            stickyHeadContainer.style.display = 'flex';
            tilesCropped.style.display = 'flex';
            tilesFullSize.style.display = 'flex';
            stickyHeadContainer.style.display = "none"
        } else if (stickyButtonsDropdown !== "true" && stickyButtonsFullSize !== "true") {
            stickyHeadContainer.style.display = "flex";
            tilesCropped.style.display = 'none';
            tilesCroppedContainer.appendChild(beforeDiv);
        } else {
            tilesCropped.style.display = 'none';
            stickyHeadContainer.style.display = "flex";
            beforeDiv.style.height = tilesCropped.offsetHeight + 'px';
            tilesCroppedContainer.appendChild(beforeDiv);
        }
        ...
        window.addEventListener('scroll', function() {
            ...
            if (scrollTop >= offsetTop) {
                ...
                if (stickyButtonsDropdown === "true" && stickyButtonsFullSize !== "true") {
                    ...
                    nextElement.style.paddingTop = beforeDiv.offsetHeight + 'px';
                }
